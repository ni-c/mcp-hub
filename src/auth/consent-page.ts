 ...
 */
export function renderConsentPage(requestToken: string, csrfToken: string, redirectUri: string, clientName?: string): string {
  return renderPage(
    'mcp-hub authorization',
    `<form method="post" action="consent">
  <h1>Authorize access?</h1>
  <p>${clientName ? `<strong>${escapeHtml(clientName)}</strong> is asking` : 'An application is asking'} for access to every server on this hub.</p>
