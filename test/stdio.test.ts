...
function startHub(configPath: string) {
  const hub = createStdioHub({ configPath });
  started.push(hub);
  return hub;
}
...
describe('stdio mode', () => {
  it('serves the same meta-tools the /hub endpoint serves', async () => {
    ...
    expect(tools.map(t => t.name).sort()).toEqual(['call_tool', 'get_tool_schema', 'list_servers', 'list_tools']);
    await client.close();
  });
